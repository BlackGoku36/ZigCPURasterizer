...
pub fn render_opaque_meshes(theta: f32) !void {
...
pub fn render_transcluent_meshes(theta: f32) !void {
    for (scene.translucent_meshes.items) |mesh| {
        if (!mesh.should_render) continue;
        // if (std.mem.eql(u8, mesh.name, "Cube")) continue;
        // const model_mat = Matrix4.multMatrix4(rot_mat, mesh.transform);
        _ = theta;
                            ...
                            while (x <= aabb.max_x) : (x += 1) {
                                if (windingOrderNone(winding_order, w_x0, w_x1, w_x2)) {
                                    ...
                                    if (z < depth_buffer.getPixel(x, y)) {
                                        ...
                                        const blend_factor: f16 = @min(transmission, 0.6);
...
pub fn render(theta: f32, camera: usize) !void {
    ...
    try render_opaque_meshes(theta);

    try render_transcluent_meshes(theta);
